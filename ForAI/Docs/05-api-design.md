...
```json
{
  ...
  "school": "示例大学",
  "company": "示例公司",
  "technicalTags": ["Go", "游戏开发", "PostgreSQL"]
}
```
...
- `technicalTags` 传入完整标签数组并替换现有值，最多 10 项、每项最长 30 个字符；后端去除首尾空格、忽略空项并按小写值去重。
- 用户技术标签与文章 `tags` 独立，不创建文章标签，也不影响搜索、推荐或排名。
- 只能更新自己的资料。
...
- `value = 1` 表示点赞，`value = 0` 表示取消点赞；其他值返回 `VALIDATION_ERROR`。
- 只能点赞已发布且所属领域、版块有效的他人文章。

响应：更新后的文章对象，包含 `upVotes` 和当前用户的 `myVote`（`0` 或 `1`）。
...
- `deleted`：是否已软删除。
- `upVotes`、`myVote`：评论点赞数和当前用户的点赞状态；`myVote` 只可能为 `0` 或 `1`。
...
- 只能对可见、未删除且所属文章已发布的评论点赞。
- 不可点赞自己的评论。
- 同一用户对同一评论最多保留一条点赞记录。

响应：更新后的评论对象，包含点赞数与当前用户 `myVote`（`0` 或 `1`）。
...
响应：当前用户发表过的评论和回复，包含所属文章标题、点赞统计、隐藏状态。
...
响应：作者公开信息（头像、用户名、bio、学校、公司、`technicalTags`、已发布文章数、关注者数、正在关注数、作者文章被收藏总数），以及已发布文章列表（分页）。文章列表额外返回每篇文章的浏览量和收藏数，用于作者主页热门文章与收藏排序展示。
...
- `GET /api/v1/admin/tasks`：登录后可访问；支持 `taskType`、`status`、`priority`、`domainId`、`moduleId`、`assigneeId`、`page`、`pageSize`。`taskType` 仅允许文章审核、内容/评论/用户举报、申诉和版块创建请求；角色由管理员通过角色管理接口直接任命或撤销。
