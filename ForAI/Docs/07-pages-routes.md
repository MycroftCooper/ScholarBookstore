...
- 登录按钮。
- 注册入口。
- 设计风格使用游学书屋品牌登录场景；第三方登录明确不做，也不展示占位入口。
...
- 前端校验两次密码一致。
- 前端要求勾选用户协议与隐私政策。
- 后端当前仅校验用户名长度、邮箱格式和密码长度至少 8 位；前端可展示弱提示，但不做复杂度强限制。
...
- 密码找回与记住登录的范围、优先级和验收标准统一记录在 `ForAI/PM/SCOPE.md` 与 `ForAI/PM/RELEASE.md`。
...
当前实现说明：
- 评论和回复 DOM 使用 `#comment-{id}` 锚点；从通知进入文章详情时可自动定位并高亮对应评论或回复。
...
- 作者资料、技术标签、已发布文章列表、关注按钮和关注统计已接真实数据。
...
- 作者主页只展示真实公开资料和已发布文章，不提供未接入数据的占位模块。文章封面已明确不做。
...
- Markdown 编辑区。
- 图片上传入口。
- Markdown 编辑区支持双栏实时预览，左侧继续按 Markdown 写作，右侧动态渲染。
- 提交按钮旁提供文章页预览，调用预览接口生成未发布文章数据，并以文章详情页布局展示醒目的“未发布 / 预览中”标记。
...
- 已登录用户在“设置”页编辑头像、Bio、学校、公司和技术标签，并预览公开资料展示。
...
- 技术标签可自由添加和删除，最多 10 项、每项最长 30 个字符；保存后在公开作者主页一致展示。
- 展示设置仍为静态展示占位，后续按 PM 已确认的四项隐私设置接入。
...
- 标记单条已读。
- 全部标记已读。
- 跳转到对应文章；评论和回复通知会带 `#comment-{id}` 定位到具体评论。
